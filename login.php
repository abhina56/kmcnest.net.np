...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT hospital_id, hospital_name, hashedPassword FROM hospital WHERE hospital_name = ?");
    $stmt->bind_param("s", $hospitalName);
    $stmt->execute();

    // Bind result variables
    $stmt->bind_result($hospital_id, $hospital_name, $storedPassword);
    while ($stmt->fetch()) {
        // Check if fetched hospital name and password match
        if ($hospital_name && $password === $storedPassword) {
            ...
            // Set cookies
            setcookie("hospital_id", $hospital_id, time() + (86400 * 30), "/");
            setcookie("hospitalName", $hospital_name, time() + (86400 * 30), "/");
  ...
  <header>
    ...
    <div class="container">
        <div class="login-container">
            <b><h2 class="text-center">Hospital Login</h2></b>
            <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                <div class="form-group">
                    <label for="hospitalName" class="form-label">Hospital Name:</label>
                    <input type="text" class="form-control" name="hospitalName" placeholder="Enter Hospital Name" required>
                </div>
