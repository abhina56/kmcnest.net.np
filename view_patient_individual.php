...
<table>
    ...
    <th>Document</th>
    <td>
        <?php if (!empty($row_patient["document_path"]) && is_array($row_patient["document_path"])): ?>
            <?php foreach ($row_patient["document_path"] as $document): ?>
                <?php if (!empty($document)): ?>
                    <a href="<?php echo htmlspecialchars($document); ?>" target="_blank"><?php echo htmlspecialchars(basename($document)); ?></a><br>
                <?php endif; ?>
            <?php endforeach; ?>
        <?php endif; ?>
    </td>
...
<div>
    ...
    <!-- Buttonfor Refering toanother hospital -->
...
<a class="btn btn-danger btn-action" onclick="confirmDeath(<?php echo $row_patient['p_id']; ?>)">Death</a>

<a href="hospital.php" class="btn btn-danger btn-action">Back</a>
...
<script>
    function confirmDischarge(p_id) {
        ...
        if (confirmAction) {
            ...
            if (dischargeDate) {
                window.location.href = "set_status.php?p_id=" + p_id + "&status=Discharged&date=" + dischargeDate;
            }
        }
    }

    function confirmDeath(p_id) {
        var confirmAction = confirm("Are you sure you want to declare this patient Death?");
        if (confirmAction) {
            var doubleCheck = confirm("Please confirm again if you want to declare the patient Death.");
            if (doubleCheck) {
                var deathDate = prompt("Please enter the death date (YYYY-MM-DD):");
                if (deathDate) {
                    window.location.href = "set_status.php?p_id=" + p_id + "&status=Death&date=" + deathDate;
                }
            }
        }
    }
